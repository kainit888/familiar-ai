...
判定順序 (この順番が仕様。allowlist が denylist に優先する):
    ...
    1. 空文字 → False
    2. 正規化後が空 → False
    3. allowlist 完全一致 → False (最優先。「ありがとう」「うん」を救う)
...
_DENYLIST: Final[tuple[str, ...]] = (
    ...
    "ありがとうございました",
    "ごめん",
    # ── Phase C-13a 追加: 実機観測の短い幻聴 (2026-05-31 11:21 セッション) ──
    # いずれも len=2、無音/雑音由来でカイニット未発話。完全一致のみで捕捉する
    # (len<5 で prefix/包含レンジに入らず、≥6 包含ゲートにも掛からないため、
    #  「いいね」「いいよ」「はいはい」等の実発話を巻き込まない)。
    "いい",
    "はい",  # C-13a で allowlist から denylist へ移動 (実は幻聴頻出)
    # ── 動画アウトロ定型句 (ご視聴系) ──
    "ご視聴ありがとうございました",
    "ご視聴いただきありがとうございました",
...
_ALLOWLIST: Final[tuple[str, ...]] = (
    ...
    "ありがとうね",
    "ありがと",
    # 「はい」は C-13a で削除 (実機で幻聴として頻出、denylist へ移動)。
    # 「うん」「ええ」「OK」は幻聴の実観測が無いため保持 (誤 drop 回避の保険)。
    "うん",
    "ええ",
    "OK",
)
